...
describe('plugin', () => describe('movie', () => {
  ...
  describe('POST /movies', () => {
    ...
    it('returns HTTP 201, with the `id` and `path` to the row created', async ({ context }: Flags) => {
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_create, payload.name, payload.synopsis, payload.releasedAt, payload.runtime)
    ...
    it('without `synopsis` attribute', async ({ context }: Flags) => {
      if(!isContext(context)) throw TypeError()
      const payload = {'name': 'any-name', 'releasedAt': new Date("2000-01-01"), 'runtime': 120}
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
      const anyResult = 123
      context.stub.lib_create.resolves(anyResult)
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_create, payload.name, undefined, payload.releasedAt, payload.runtime)
      expect(response.result).equals({
        id: anyResult,
        path: `/movies/${anyResult}`
      })
  ...
  describe('PUT /movies/:id', () => {
    const paramId = 123
    const [method, url, payload] = ['PUT', `/movies/${paramId}`, {
      'name': 'any-name', 
    ...
    it('returns HTTP 204', async ({ context }: Flags) => {
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_update, paramId, payload.name, payload.synopsis, payload.releasedAt, payload.runtime)
      expect(response.result).to.be.null()
    })

    it('without `synopsis` attribute', async ({ context }: Flags) => {
      if(!isContext(context)) throw TypeError()
      const { synopsis: dummyValue, ...payloadSubset } = payload;
      const opts: Hapi.ServerInjectOptions = { method, url, payload: payloadSubset }
      const anyResult = 123
      context.stub.lib_update.resolves(anyResult)
      ...
      sinon.assert.calledOnceWithExactly(context.stub.lib_update, paramId, payload.name, undefined, payload.releasedAt, payload.runtime)
      expect(response.result).to.be.null()
    })
